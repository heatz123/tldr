...
def get_model_and_assets(color=False):
    """Returns a tuple containing the model XML string and a dict of assets."""
    root_dir = os.path.dirname(os.path.dirname(__file__))
    if color:
        xml = resources.GetResource(
            os.path.join(root_dir, "custom_dmc_tasks", "cheetah_color.xml")
        )
    else:
        xml = resources.GetResource(
...
def run_forward_color(
    ...
    """Returns the run task."""
    physics = Physics.from_xml_string(*get_model_and_assets(color=True))
    task = Cheetah(forward=True, flip=False, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs
