...
def _make_dmc(
    obs_type, domain, task, frame_stack, action_repeat, seed, task_kwargs=None
):
...
def make(name, obs_type, frame_stack, action_repeat, seed, task_kwargs=None):
    ...
    camera_id = None
    if "color" not in name and domain in ["quadruped", "humanoid"]:
        if domain in ["quadruped"]:
            camera_id = 3
        elif domain in ["humanoid"]:
            camera_id = 1
    env = DMCGymWrapper(env, domain=domain, camera_id=camera_id)
