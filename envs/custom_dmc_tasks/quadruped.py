...
def make_model(
    ...
    if not steps and not color:
        _STEPS = [
            "step_px",
            "step_nx",
            *[f"{j}step_{i}" for i in range(1, 9) for j in ["", "n"]],
        ]
        for step in _STEPS:
            terrain_geom = xml_tools.find_element(mjcf, "geom", step)
            terrain_geom.getparent().remove(terrain_geom)
...
@SUITE.add()
def step(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Run task."""
    xml_string = make_model(floor_size=_DEFAULT_TIME_LIMIT * _RUN_SPEED, steps=True)
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
    task = GoFar(random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
@SUITE.add()
def step_bw(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Run task."""
    xml_string = make_model(
        floor_size=_DEFAULT_TIME_LIMIT * _RUN_SPEED, steps=True, walls_and_ball=True
    )
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
    task = Move(desired_speed=_RUN_SPEED, random=random)
    environment_kwargs = environment_kwargs or {}
...
@SUITE.add()
def escapep(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Escape task."""
...
class Move(base.Task):
    ...
    def initialize_episode(self, physics):
        ...
        orientation = (
            np.array([1, 0, 0, 0]) + self.random.randn(4) * 0.02
...
class GoFar(base.Task):
    ...
    def get_reward(self, physics):
        ...
        # Move reward term.
        reward = np.sqrt(np.sum(physics.origin_distance() ** 2))
...
class Escape(base.Task):
    ...
    def initialize_episode(self, physics):
        ...
        # Set the seed for reproducibility of uniform random numbers
        np.random.seed(0)
