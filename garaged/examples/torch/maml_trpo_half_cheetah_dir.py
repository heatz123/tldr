...
def maml_trpo_half_cheetah_dir(ctxt, seed, epochs, rollouts_per_task, meta_batch_size):
    ...
    """
    set_seed(seed)
    env = GarageEnv(normalize(HalfCheetahDirEnv(), expected_action_scale=10.0))
    ...
    task_sampler = SetTaskSampler(
        lambda: GarageEnv(normalize(HalfCheetahDirEnv(), expected_action_scale=10.0))
    )
    ...
    runner.train(n_epochs=epochs, batch_size=rollouts_per_task * max_path_length)
