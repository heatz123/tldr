...
def maml_trpo_metaworld_ml1_push(
    ...
    set_seed(seed)
    env = GarageEnv(
        normalize(mwb.ML1.get_train_tasks("push-v1"), expected_action_scale=10.0)
    )
    ...
    test_sampler = SetTaskSampler(
        lambda: GarageEnv(normalize(mwb.ML1.get_test_tasks("push-v1")))
    )

    meta_evaluator = MetaEvaluator(
        test_task_sampler=test_sampler, max_path_length=max_path_length
    )
