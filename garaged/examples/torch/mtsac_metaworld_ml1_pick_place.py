...
def mtsac_metaworld_ml1_pick_place(ctxt=None, seed=1, _gpu=None):
    ...
    for i in range(50):
        train_env = GarageEnv(
            normalize(mwb.ML1.get_train_tasks("pick-place-v1"), normalize_reward=True)
        )
        test_env = pickle.loads(pickle.dumps(train_env))
        env_names.append("pick_place_{}".format(i))
        train_envs.append(train_env)
        test_envs.append(test_env)
    ml1_train_envs = MultiEnvWrapper(
        train_envs, sample_strategy=round_robin_strategy, env_names=env_names
    )
    ...
    mtsac.to()
    runner.setup(algo=mtsac, env=ml1_train_envs, sampler_cls=LocalSampler, n_workers=1)
