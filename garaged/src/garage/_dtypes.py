    ...
    def __new__(
        ...
        for key, val in env_infos.items():
            ...
            if isinstance(val, np.ndarray) and val.shape[0] != inferred_batch_size:
                if not (
                    val.shape[0] == len(lengths)
                    and sum([len(v) for v in val]) == inferred_batch_size
                ):
                    raise ValueError(
    ...
    def __new__(
        ...
        if not env_spec.observation_space.contains(observation):
            if isinstance(
                ...
                if env_spec.observation_space.flat_dim != np.prod(observation.shape):
                    raise ValueError(
                        "observation should have the same "
                        "dimensionality as the observation_space "
                        "({}), but got data with shape {} "
                        "instead".format(
                            env_spec.observation_space.flat_dim, observation.shape
                        )
                    )
            else:
                raise ValueError(
        ...
        if not env_spec.observation_space.contains(next_observation):
            if isinstance(
                ...
                if env_spec.observation_space.flat_dim != np.prod(
                    ...
                    raise ValueError(
                        "next_observation should have the same "
                        "dimensionality as the observation_space "
                        "({}), but got data with shape {} "
                        "instead".format(
                            env_spec.observation_space.flat_dim, next_observation.shape
                        )
                    )
            else:
                raise ValueError(
        ...
        if not env_spec.action_space.contains(action):
            if isinstance(env_spec.action_space, (akro.Box, akro.Discrete, akro.Dict)):
                if env_spec.action_space.flat_dim != np.prod(action.shape):
                    raise ValueError(
                        "action should have the same "
                        "dimensionality as the action_space "
                        "({}), but got data with shape {} "
                        "instead".format(env_spec.action_space.flat_dim, action.shape)
                    )
            else:
                raise ValueError(
                    "action must conform to action_space {}, "
                    "but got data with shape {} instead.".format(
        ...
        if not isinstance(agent_info, dict):
            raise ValueError(
                "agent_info must be type {}, but got type {} "
                "instead.".format(dict, type(agent_info))
            )

        if not isinstance(env_info, dict):
            raise ValueError(
                "env_info must be type {}, but got type {} "
        ...
        if not isinstance(reward, float):
            raise ValueError(
                "reward must be type {}, but got type {} "
                "instead.".format(float, type(reward))
            )

        if not isinstance(terminal, bool):
            raise ValueError(
                "terminal must be dtype bool, but got dtype {} instead.".format(
                    type(terminal)
                )
            )
    ...
    def __new__(
        ...
        if not env_spec.observation_space.contains(next_observations[0]):
            if isinstance(
                ...
                if env_spec.observation_space.flat_dim != np.prod(
                    ...
                    raise ValueError(
                        "next_observations should have the same "
                        "dimensionality as the observation_space "
                        "({}), but got data with shape {} "
                        "instead".format(
                            env_spec.observation_space.flat_dim,
                ...
                raise ValueError(
                    ...
                    "but got data with shape {} instead.".format(
                        env_spec.observation_space, next_observations[0].shape[0]
                    )
                )
        ...
        if not env_spec.action_space.contains(first_action):
            if isinstance(env_spec.action_space, (akro.Box, akro.Discrete, akro.Dict)):
                ...
                raise ValueError(
                    "actions must conform to action_space {}, "
                    "but got data with shape {} instead.".format(
                        env_spec.action_space, first_action.shape
                    )
                )
        if actions.shape[0] != inferred_batch_size:
            raise ValueError(
        ...
        if rewards.shape[0] != inferred_batch_size:
            raise ValueError(
                ...
                "length {} instead.".format(inferred_batch_size, rewards.shape[0])
            )
