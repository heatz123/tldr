...
class GarageEnv(gym.Wrapper):
    ...
    def __init__(self, env=None, env_name="", is_image=False):
        ...
        self.observation_space = akro.from_gym(
            self.env.observation_space, is_image=is_image
        )
        self.__spec = EnvSpec(
    ...
    def _close_viewer_window(self):
        ...
        if self.env.spec:
            if any(
                package in getattr(self.env.spec, "entry_point", "")
                ...
                try:
                    ...
                    return
                if hasattr(self.env, "viewer") and isinstance(
                    self.env.viewer, MjViewer
                ):
                ...
                if hasattr(self.env, "viewer"):
                    ...
                    if isinstance(self.env.viewer, (SimpleImageViewer, Viewer)):
