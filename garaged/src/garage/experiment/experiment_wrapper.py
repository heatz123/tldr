...
def run_experiment(argv):
    ...
    if args.log_dir is None:
        log_dir = os.path.join(os.path.join(os.getcwd(), "data"), args.exp_name)
    else:
    ...
    logger.add_output(dowel.TensorBoardOutput(log_dir, x_axis="TotalEpoch"))
    logger.add_output(dowel.StdOutput())

    logger.push_prefix("[%s] " % args.exp_name)

    snapshot_config = garage.experiment.SnapshotConfig(
    ...
    try:
        method_call(
            snapshot_config, variant_data, args.resume_from_dir, args.resume_from_epoch
        )
...
def child_proc_shutdown(children):
    ...
    for proc in alive:
        if any(["multiprocessing.semaphore_tracker" in cmd for cmd in proc.cmdline()]):
            alive.remove(proc)
    ...
    if alive:
        ...
        for child in alive:
            error_msg += "{}\n".format(
                str(child.as_dict(attrs=["ppid", "pid", "name", "status", "cmdline"]))
            )

        error_msg = (
            "The following processes didn't die after the shutdown "
            "of run_experiment:\n"
        ) + error_msg
        error_msg += (
            "This is a sign of an unclean shutdown. Please reopen "
