        ...
        for path in samples:
            ...
            if hist_variance_penalty > 0:
                ...
                dist2 = tfp.distributions.MultivariateNormalDiag(
                    loc=dist2.loc,
                    scale_diag=dist2.scale._diag + log(1.0 + hist_variance_penalty),
                )
            ...
            loglike_p = tf.compat.v1.get_default_session().run(
                ...
                feed_dict={policy.model.input: np.expand_dims(path["observations"], 1)},
            )
...
class __FakeISSampler:
    ...
    def __init__(*args, **kwargs):
        raise ImportError(
            "ISSampler requires TensorFlow. To use it, please install " "TensorFlow."
        )
