...
class OffPolicyVectorizedSampler(BatchSampler):
    ...
    def obtain_samples(self, itr, batch_size=None, whole_paths=True):
        ...
        if policy is None:
            raise ValueError(
                "OffPolicyVectoriizedSampler should only be used "
        ...
        while n_samples < batch_size:
            ...
            next_obses, rewards, dones, env_infos, completes = self._vec_env.step(
                actions
            )
            ...
            for idx, reward, env_info, done, complete, obs, next_obs, action in zip(
                itertools.count(),
                rewards,
                env_infos,
                dones,
                ...
                running_paths[idx]["success_count"] += env_info.get("is_success") or 0

                self._last_uncounted_discount[idx] += reward
                self._last_success_count[idx] += env_info.get("is_success") or 0
                ...
                if complete or n_samples >= batch_size:
                    paths.append(
                        dict(
                            ...
                            running_length=running_paths[idx]["running_length"],
