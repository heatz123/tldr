...
def terminate_task(scope=None):
    ...
    singleton_pool.run_each(
        _worker_terminate_task, [(scope,)] * singleton_pool.n_parallel
    )
...
def set_seed(seed):
    ...
    singleton_pool.run_each(
        _worker_set_seed, [(seed + i,) for i in range(singleton_pool.n_parallel)]
    )
...
def sample_paths(policy_params, max_samples, max_path_length=np.inf, scope=None):
    ...
    """
    singleton_pool.run_each(
        _worker_set_policy_params, [(policy_params, scope)] * singleton_pool.n_parallel
    )
