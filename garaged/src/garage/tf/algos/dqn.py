        ...
        with tf.name_scope(self._name):
            ...
            with tf.name_scope("td_error"):
                ...
                q_selected = tf.reduce_sum(
                    self._qf.q_vals * action, axis=1  # yapf: disable
                )
    ...
    def train_once(self, itr, paths):
        ...
        if itr % self._steps_per_epoch == 0:
            if self.replay_buffer.n_transitions_stored >= self._min_buffer_size:
                ...
                tabular.record(
                    "{}/Episode100LossMean".format(self._qf.name), mean100ep_qf_loss
                )
    ...
    def optimize_policy(self, samples_data):
        ...
        if isinstance(self.env_spec.observation_space, akro.Image):
            if len(observations.shape[1:]) < len(self.env_spec.observation_space.shape):
                ...
                next_observations = self.env_spec.observation_space.unflatten_n(
                    next_observations
                )
