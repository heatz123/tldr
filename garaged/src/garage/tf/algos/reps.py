...
class REPS(RLAlgorithm):  # noqa: D416
    ...
    def train_once(self, itr, paths):
        ...
        paths = [
            dict(
                ...
                actions=(
                    self._env_spec.action_space.flatten_n(path["actions"])  # noqa: E126
                ),
        ...
        if hasattr(self._baseline, "predict_n"):
            ...
            baseline_predictions = [self._baseline.predict(path) for path in paths]
    ...
    def optimize_policy(self, samples_data):
        ...
        params_ast, _, _ = self._dual_optimizer(
            func=eval_dual,
            x0=x0,
    ...
    def _build_inputs(self):
        ...
        with tf.name_scope("inputs"):
            ...
            valid_var = tensor_utils.new_tensor(
                name="valid", ndim=2, dtype=tf.float32
            )  # yapf: disable
            feat_diff = tensor_utils.new_tensor(
                name="feat_diff", ndim=2, dtype=tf.float32
            )  # yapf: disable
            param_v = tensor_utils.new_tensor(
                name="param_v", ndim=1, dtype=tf.float32
            )  # yapf: disable
            param_eta = tensor_utils.new_tensor(
                name="param_eta", ndim=0, dtype=tf.float32
            )  # yapf: disable
        ...
        self._old_policy_network = self._old_policy.build(obs_var, name="policy")
    ...
    def _build_policy_loss(self, i):
        ...
        with tf.name_scope("policy_loss"):
            ...
            loss = -tf.reduce_mean(
                ll
                * tf.exp(delta_v / i.param_eta - tf.reduce_max(delta_v / i.param_eta))
            )
            ...
            loss += (
                self._l2_reg_loss
                * tf.reduce_sum(
                    [tf.reduce_mean(tf.square(param)) for param in reg_params]
                )
                / len(reg_params)
            )
        ...
        self._f_dual_grad = tensor_utils.compile_function(
            flatten_inputs(self._dual_opt_inputs), dual_grad, log_name="f_dual_grad"
        )

        self._f_policy_kl = tensor_utils.compile_function(
            flatten_inputs(self._policy_opt_inputs), pol_mean_kl, log_name="f_policy_kl"
        )
    ...
    def _features(self, samples_data):
        ...
        for path in paths:
            ...
            feats = np.concatenate(
                [o, o**2, al, al**2, al**3, np.ones((lr, 1))], axis=1
            )
