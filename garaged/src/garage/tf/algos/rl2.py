...
class RL2Worker(DefaultWorker):
    ...
    def rollout(self):
        ...
        self._agent_infos["batch_idx"] = np.full(
            len(self._rewards), self._worker_number
        )
...
class RL2(MetaRLAlgorithm, abc.ABC):
    ...
    def _process_samples(self, itr, paths):
        ...
        concatenated_paths_stacked = np_tensor_utils.stack_and_pad_tensor_dict_list(
        ...
        if hasattr(self._task_sampler, "_envs") and hasattr(
            ...
            names = [env.env.all_task_names[0] for env in self._task_sampler._envs]
        ...
        concatenated_paths_stacked["average_return"] = np.mean(undiscounted_returns)
    ...
    def _concatenate_paths(self, paths):
        ...
        actions = np.concatenate(
            [self.env_spec.action_space.flatten_n(path["actions"]) for path in paths]
        )
        valids = np.concatenate([np.ones_like(path["rewards"]) for path in paths])
        baselines = np.concatenate([np.zeros_like(path["rewards"]) for path in paths])
