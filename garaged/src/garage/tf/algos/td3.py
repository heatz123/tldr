...
class TD3(RLAlgorithm):
    ...
    def init_opt(self):
        ...
        with tf.name_scope(self._name):
            ...
            target_qf_outputs = self._target_qf.get_qval_sym(obs, actions, name="qf")
            target_qf2_outputs = self._target_qf2.get_qval_sym(obs, actions, name="qf")
        ...
        for _ in runner.step_epochs():
            for cycle in range(self._steps_per_epoch):
                ...
                if (
                    ...
                    log_performance(
                        runner.step_itr,
                        obtain_evaluation_samples(self.policy, runner.get_env_copy()),
    ...
    def train_once(self, itr, paths):
        ...
        self._episode_rewards.extend(
            [
                ...
                for path, complete in zip(
                    paths["undiscounted_returns"], paths["complete"]
                )
                if complete
            ]
        )
        self._success_history.extend(
            [
                path
                for path, complete in zip(paths["success_history"], paths["complete"])
                if complete
            ]
        )
    ...
    def optimize_policy(self, itr):
        ...
        target_q2vals = self.target_qf2_f_prob_online(next_inputs, target_actions)
