...
class TENPO(RLAlgorithm):
    ...
    def optimize_policy(self, itr, samples_data):
        ...
        inference_opt_input_values = self._inference_opt_input_values(samples_data)
    ...
    def paths_to_tensors(self, paths):
        ...
        for path in paths:
            if self._flatten_input:
                path["observations"] = self._env_spec.observation_space.flatten_n(
                    path["observations"]
                )
            path["actions"] = self._env_spec.action_space.flatten_n(path["actions"])
        ...
        samples_data = paths_to_tensors(
            paths, max_path_length, all_path_baselines, self._discount, self._gae_lambda
        )
    ...
    def _build_inputs(self):
        ...
        with tf.name_scope("inputs"):
            if self._flatten_input:
                ...
                task_var = task_space.to_tf_placeholder(name="task", batch_dims=2)
                trajectory_var = trajectory_space.to_tf_placeholder(
                    name="trajectory", batch_dims=2
                )
                latent_var = latent_space.to_tf_placeholder(name="latent", batch_dims=2)
    ...
    def _build_policy_loss(self, i):
        ...
        with tf.name_scope("policy_loss"):
            ...
            returns = discounted_returns(
                self._discount, self.max_path_length, rewards, name="returns"
            )
    ...
    def _build_inference_loss(self, i):
        ...
        with tf.name_scope("infer_loss"):
            ...
            traj_gammas = tf.constant(
                float(self._discount), dtype=tf.float32, shape=[self.max_path_length]
            )
            # Pylint false alarm
    ...
    def _policy_opt_input_values(self, samples_data):
        ...
        embed_state_info_list = [
            samples_data["latent_infos"][k] for k in self.policy.encoder.state_info_keys
        ]
    ...
    def _inference_opt_input_values(self, samples_data):
        ...
        infer_state_info_list = [
            samples_data["trajectory_infos"][k] for k in self._inference.state_info_keys
        ]
    ...
    def evaluate(self, policy_opt_input_values, samples_data):
        ...
        env_average_discounted_return = np.mean([path["returns"][0] for path in paths])

        # Recompute parts of samples_data
        aug_rewards = []
        aug_returns = []
        for rew, ret, val, path in zip(rewards_tensor, returns_tensor, valids, paths):
            path["rewards"] = rew[val.astype(bool)]
            path["returns"] = ret[val.astype(bool)]
        ...
        aug_average_discounted_return = np.mean([path["returns"][0] for path in paths])
        d_returns = np.mean(
        ...
        inference_rmse = (
            samples_data["trajectory_infos"]["mean"] - samples_data["latents"]
        ) ** 2.0
        ...
        for t in range(self.policy.task_space.flat_dim):
            ...
            tabular.record("Tasks/CompletionRate/t={}".format(t), pct_completed)
