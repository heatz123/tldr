...
class Categorical(Distribution):
        ...
        with tf.compat.v1.variable_scope(name, "Categorical"):
            ...
            self._f_sample = compile_function(
                ...
                outputs=tf.random.categorical(
                    tf.math.log(weights_var + 1e-8), num_samples=1
                )[:, 0],
            )
    ...
    def kl_sym(self, old_dist_info_vars, new_dist_info_vars, name="kl_sym"):
        ...
        with tf.name_scope(name):
            ...
            return tf.reduce_sum(
                old_prob_var
                * (tf.math.log(old_prob_var + TINY) - tf.math.log(new_prob_var + TINY)),
                axis=ndims - 1,
            )
    ...
    def kl(self, old_dist_info, new_dist_info):
        ...
        return np.sum(
            old_prob * (np.log(old_prob + TINY) - np.log(new_prob + TINY)), axis=-1
        )
    ...
    def cross_entropy_sym(
        self, old_dist_info_vars, new_dist_info_vars, name="cross_entropy_sym"
    ):
        with tf.name_scope(name):
            old_prob_var = old_dist_info_vars["prob"]
            new_prob_var = new_dist_info_vars["prob"]
            ...
            # Assume layout is N * A
            return tf.reduce_sum(
                old_prob_var * (-tf.math.log(new_prob_var + TINY)), axis=ndims - 1
            )
    ...
    def log_likelihood_sym(self, x_var, dist_info_vars, name="log_likelihood_sym"):
        with tf.name_scope(name):
            ...
            return tf.math.log(
                tf.reduce_sum(probs * tf.cast(x_var, tf.float32), ndims - 1) + TINY
            )
    ...
    def sample_sym(self, dist_info, name="sample_sym"):
        with tf.name_scope(name):
            probs = dist_info["prob"]
            samples = tf.multinomial(tf.math.log(probs + 1e-8), num_samples=1)[:, 0]
