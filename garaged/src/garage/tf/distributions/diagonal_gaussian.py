...
class DiagonalGaussian(Distribution):
    ...
    def kl(self, old_dist_info, new_dist_info):
        ...
        # ln(\sigma_2/\sigma_1)
        numerator = (
            np.square(old_means - new_means) + np.square(old_std) - np.square(new_std)
        )
    ...
    def sample_sym(self, dist_info_vars):
        ...
        rnd = tf.random.normal(
            shape=tf.shape(means), seed=deterministic.get_tf_seed_stream()
        )
