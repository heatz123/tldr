...
class CategoricalGRUPolicy(StochasticPolicy):
    ...
    def __init__(
        ...
        if not isinstance(env_spec.action_space, akro.Discrete):
            raise ValueError(
                "CategoricalGRUPolicy only works" "with akro.Discrete action space."
            )
    ...
    def _initialize(self):
        ...
        with tf.compat.v1.variable_scope(self.name) as vs:
            ...
            (self._dist, step_out, step_hidden, self._init_hidden) = self.model.build(
                state_input, step_input_var, step_hidden_var
            ).outputs
    ...
    def build(self, state_input, name=None):
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            ...
            return self.model.build(
                state_input, step_input_var, step_hidden_var, name=name
            )
