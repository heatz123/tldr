    ...
    def __init__(
        ...
        with tf.compat.v1.variable_scope(self._name, reuse=False) as vs:
            ...
            if tr_optimizer is None:
                self._tr_optimizer = make_optimizer(
                    ConjugateGradientOptimizer, **tr_optimizer_args
                )
            else:
    ...
    def _initialize(self):
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            ...
            old_prob_var = tf.compat.v1.placeholder(
                dtype=tf.float32, name="old_prob", shape=(None, self._output_dim)
            )
            ...
            self._optimizer.update_opt(
                loss=loss, target=self, inputs=[input_var, ys_var]
            )
