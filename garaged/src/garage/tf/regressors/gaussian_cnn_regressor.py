...
from garage.tf.regressors.gaussian_cnn_regressor_model import GaussianCNNRegressorModel
    ...
    def _initialize(self):
        ...
        with tf.compat.v1.variable_scope(self._variable_scope):
            ...
            old_means_var = tf.compat.v1.placeholder(
                dtype=tf.float32, name="old_means", shape=(None, self._output_dim)
            )
            old_log_stds_var = tf.compat.v1.placeholder(
                dtype=tf.float32, name="old_log_stds", shape=(None, self._output_dim)
            )
