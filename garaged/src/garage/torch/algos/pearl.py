...
class PEARL(MetaRLAlgorithm):
        ...
        for _ in runner.step_epochs():
            ...
            if epoch == 0 or self._is_resuming:
                for idx in range(self._num_train_tasks):
                    self._task_idx = idx
                    self._obtain_samples(runner, epoch, self._num_initial_steps, np.inf)
                    self._is_resuming = False
            ...
            for _ in range(self._num_tasks_sample):
                ...
                if self._num_steps_prior > 0:
                    self._obtain_samples(runner, epoch, self._num_steps_prior, np.inf)
    ...
    def _obtain_samples(
        self, runner, itr, num_samples, update_posterior_rate, add_to_enc_buffer=True
    ):
        """Obtain samples.
