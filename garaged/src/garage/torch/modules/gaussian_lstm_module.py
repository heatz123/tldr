...
class GaussianLSTMBaseModule(nn.Module):  # this only defines head
    ...
    def forward(self, *inputs, hidden_states=None):
        ...
        """
        (mean, log_std_uncentered), hidden_states = (
            self._get_mean_and_log_std_with_hidden_states(
        ...
        if self._std_parameterization not in ["softplus_real"]:
            if self._min_std_param or self._max_std_param:
                log_std_uncentered = log_std_uncentered.clamp(
                    min=(
                        None
                        if self._min_std_param is None
                        else self._min_std_param.item()
                    ),
                    max=(
                        None
                        if self._max_std_param is None
                        else self._max_std_param.item()
                    ),
                )
        ...
        if self._std_parameterization == "exp":
            ...
            std = log_std_uncentered.exp().add(1.0).log()
        else:
            assert False
    ...
    def reset(self, dones=None):
        # reset lstm hidden state, if exists

        for module in self.modules():
            if isinstance(module, LSTMModule) or isinstance(
                module, MultiHeadedLSTMModule
            ):
                module.reset(dones=dones)
        return


class GaussianLSTMModule(GaussianLSTMBaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(GaussianLSTMModule, self).__init__(
            input_dim=input_dim,
    ...
    def _get_mean_and_log_std(self, *inputs, hidden_state=None):
        ...
        uncentered_log_std = (
            torch.zeros(*broadcast_shape, device=self._init_std.device) + self._init_std
        )
...
class GaussianLSTMIndependentStdModule(GaussianLSTMBaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(GaussianLSTMIndependentStdModule, self).__init__(
            input_dim=input_dim,
            output_dim=output_dim,
            hidden_sizes=hidden_sizes,
            hidden_nonlinearity=hidden_nonlinearity,
        ...
        self._log_std_module = LSTMModule(
            input_dim=self._input_dim,
            output_dim=self._action_dim,
    ...
    def _init_std_b(self, b):
        ...
        if self._std_parameterization not in ["softplus_real"]:
            ...
            return nn.init.constant_(
                b, self._init_std.exp().exp().add(-1.0).log().item()
            )
    ...
    def _get_mean_and_log_std(self, *inputs):
        ...
        """
        return self._mean_module(*inputs)[0], self._log_std_module(*inputs)[0]

    def _get_mean_and_log_std_with_hidden_states(self, *inputs, hidden_states=None):
        """Get mean and std of Gaussian distribution given inputs.
        ...
        if hidden_states is not None:
            mean_hidden_states, log_std_hidden_states = hidden_states
        else:
            mean_hidden_states = log_std_hidden_states = None
        mean, mean_hidden_states = self._mean_module(
            *inputs, hidden_states=mean_hidden_states
        )
        log_std, log_std_hidden_states = self._log_std_module(
            *inputs, hidden_states=log_std_hidden_states
        )
        return (mean, log_std), (mean_hidden_states, log_std_hidden_states)
...
class GaussianLSTMTwoHeadedModule(GaussianLSTMBaseModule):
    ...
    def __init__(
        ...
        super(GaussianLSTMTwoHeadedModule, self).__init__(
            input_dim=input_dim,
            output_dim=output_dim,
            hidden_sizes=hidden_sizes,
            hidden_nonlinearity=hidden_nonlinearity,
        ...
        self._shared_mean_log_std_network = MultiHeadedLSTMModule(
            n_heads=2,
            input_dim=self._input_dim,
            ...
            output_b_inits=[
                ...
                (
                    lambda x: (
                        nn.init.constant_(x, self._init_std.item())
                        if self._std_parameterization not in ["softplus_real"]
                        else lambda x: nn.init.constant_(
                            x, self._init_std.exp().exp().add(-1.0).log().item()
                        )
                    )
                ),
            ],
    ...
    def _get_mean_and_log_std_with_hidden_states(self, *inputs, hidden_states=None):
        ...
        """
        return self._shared_mean_log_std_network(*inputs, hidden_states=hidden_states)
