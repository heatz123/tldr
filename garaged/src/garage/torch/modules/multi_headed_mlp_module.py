...
class MultiHeadedMLPModule(nn.Module):
    ...
    def __init__(
        ...
        for size in hidden_sizes:
            ...
            if spectral_normalization:
                linear_layer = spectral_norm(
                    nn.Linear(prev_size, size, bias=bias), spectral_coef=spectral_coef
                )
            else:
        ...
        # # hidden_layers = nn.Sequential()
        # # if spectral_normalization:
        # #     linear_layer = spectral_norm(nn.LSTM(prev_size, size, num_layers=len(hidden_sizes), bias=bias, batch_first=True), spectral_coef=spectral_coef)
        # # else:
        # #     linear_layer = nn.LSTM(prev_size, size, num_layers=len(hidden_sizes), bias=bias, batch_first=True)
    ...
    def forward(self, input_val):
        ...
        #     input_val = self.lstm(input_val)[0]
        #     input_val = input_val.squeeze(1)
        # #### debug
