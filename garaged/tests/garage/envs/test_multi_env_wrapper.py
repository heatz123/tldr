...
class TestMultiEnvWrapper:
    ...
    def _init_multi_env_wrapper(
        self, env_names, sample_strategy=uniform_random_strategy
    ):
    ...
    def test_one_hot_observation_space(self):
        ...
        obs_space = akro.Box(
            np.concatenate([cartpole_lb, np.zeros(2)]),
            np.concatenate([cartpole_ub, np.ones(2)]),
        )
        assert mt_env.observation_space.shape == obs_space.shape
        assert (mt_env.observation_space.bounds[0] == obs_space.bounds[0]).all()
        assert (mt_env.observation_space.bounds[1] == obs_space.bounds[1]).all()
    ...
    def test_one_hot_observation(self):
        ...
        obs = mt_env.reset()
        assert (obs[-2:] == np.array([1.0, 0.0])).all()
        obs = mt_env.step(1)[0]
        assert (obs[-2:] == np.array([1.0, 0.0])).all()

        obs = mt_env.reset()
        assert (obs[-2:] == np.array([0.0, 1.0])).all()
        obs = mt_env.step(1)[0]
        assert (obs[-2:] == np.array([0.0, 1.0])).all()
...
class TestMetaWorldMultiEnvWrapper:
    ...
    def setup_class(self):
        ...
        self.env = MultiEnvWrapper(
            envs, sample_strategy=round_robin_strategy, mode="vanilla", env_names=tasks
        )
