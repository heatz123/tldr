...
def test_meta_evaluator():
    ...
    with tempfile.TemporaryDirectory() as log_dir_name:
        runner = LocalRunner(
            SnapshotConfig(
                snapshot_dir=log_dir_name, snapshot_mode="last", snapshot_gap=1
            )
        )
        ...
        meta_eval = MetaEvaluator(
            test_task_sampler=tasks, max_path_length=max_path_length, n_test_tasks=10
        )
...
def test_meta_evaluator_with_tf():
    ...
    with tempfile.TemporaryDirectory() as log_dir_name:
        ctxt = SnapshotConfig(
            snapshot_dir=log_dir_name, snapshot_mode="none", snapshot_gap=1
        )
