...
class TestResume(TfGraphTestCase):
    ...
    def setup_method(self):
        ...
        self.snapshot_config = SnapshotConfig(
            snapshot_dir=self.temp_dir.name, snapshot_mode="last", snapshot_gap=1
        )
    ...
    def test_resume(self):
        ...
        with LocalTFRunner(self.snapshot_config, sess) as runner:
            ...
            assert np.equal(
                runner._algo.policy.get_param_values(), self.policy_params
            ).all(), "Policy parameters should persist"
            assert args.n_epochs == 5, "Snapshot should save training parameters"
            assert args.start_epoch == 5, "Last experiment should end at 5th iterations"

            batch_size = runner._train_args.batch_size

            runner.resume(
                n_epochs=10, plot=False, store_paths=True, pause_for_plot=False
            )
