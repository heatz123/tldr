...
class TestHerReplayBuffer:
    ...
    def _add_one_path(self):
        path = dict(
            ...
            actions=np.asarray(
                [self.env.action_space.sample(), self.env.action_space.sample()]
            ),
    ...
    def test_add_path(self):
        ...
        total_expected_transitions = (
            sum([self._replay_k + 1 for _ in range(path_len - 1)]) + 1
        )
    ...
    def test_pickleable(self):
        ...
        for k in replay_buffer_pickled._buffer:
            assert (
                replay_buffer_pickled._buffer[k].shape
                == self.replay_buffer._buffer[k].shape
            )
