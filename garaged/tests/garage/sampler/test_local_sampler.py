...
class TestSampler:
    ...
    def setup_method(self):
        ...
        self.algo = Mock(env_spec=self.env.spec, policy=self.policy, max_path_length=16)

    def teardown_method(self):
        self.env.close()

    def test_local_batch_sampler(self):
        workers = WorkerFactory(seed=100, max_path_length=self.algo.max_path_length)
        sampler1 = LocalSampler.from_worker_factory(workers, self.policy, self.env)
        ...
        assert (
            sum(trajs1.rewards[: trajs1.lengths[0]]) == sum(trajs2[0]["rewards"]) == 1
        )
...
def test_update_envs_env_update():
    ...
    policy = FixedPolicy(
        env.spec,
        scripted_actions=[env.action_space.sample() for _ in range(max_path_length)],
    )
    tasks = SetTaskSampler(PointEnv)
    n_workers = 8
    workers = WorkerFactory(
        seed=100, max_path_length=max_path_length, n_workers=n_workers
    )
    sampler = LocalSampler.from_worker_factory(workers, policy, env)
    rollouts = sampler.obtain_samples(
        0,
        161,
...
def test_obtain_exact_trajectories():
    ...
    n_traj_per_worker = 3
    rollouts = sampler.obtain_exact_trajectories(
        n_traj_per_worker, agent_update=policies
    )
...
def test_no_seed():
    ...
    n_workers = 8
    workers = WorkerFactory(
        seed=None, max_path_length=max_path_length, n_workers=n_workers
    )
    sampler = LocalSampler.from_worker_factory(workers, policy, env)
    rollouts = sampler.obtain_samples(0, 160, policy)
    assert sum(rollouts.lengths) >= 160
