...
def test_obtain_samples():
    ...
    workers = WorkerFactory(seed=100, max_path_length=algo.max_path_length, n_workers=8)
    ...
    trajs1 = sampler1.obtain_samples(0, 1000, tuple(algo.policy.get_param_values()))
    trajs2 = sampler2.obtain_samples(0, 1000, tuple(algo.policy.get_param_values()))
    # pylint: disable=superfluous-parens
    assert trajs1.observations.shape[0] >= 1000
    assert trajs1.actions.shape[0] >= 1000
    assert (
        sum(trajs1.rewards[: trajs1.lengths[0]])
        == sum(trajs2.rewards[: trajs2.lengths[0]])
        == 1
    )
...
def test_pickle():
    ...
    rollouts = sampler2.obtain_samples(
        0,
        161,
        np.asarray(policy.get_param_values()),
        env_update=tasks.sample(n_workers),
