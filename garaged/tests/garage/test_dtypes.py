...
def traj_data():
    # spaces
    obs_space = gym.spaces.Box(low=1, high=np.inf, shape=(4, 3, 2), dtype=np.float32)
...
def test_lengths_shape_mismatch_traj(traj_data):
    with pytest.raises(ValueError, match="Lengths tensor must be a tensor of shape"):
...
def test_lengths_dtype_mismatch_traj(traj_data):
    with pytest.raises(ValueError, match="Lengths tensor must have an integer dtype"):
...
def test_last_obs_batch_mismatch_traj(traj_data):
    with pytest.raises(ValueError, match="batch dimension of last_observations"):
        traj_data["last_observations"] = traj_data["last_observations"][:-1]
        t = TrajectoryBatch(**traj_data)
        del t
...
def test_act_box_env_spec_mismatch_traj(traj_data):
    with pytest.raises(ValueError, match="actions should have"):
        traj_data["env_spec"].action_space = akro.Box(
            low=1, high=np.inf, shape=(4, 3, 2), dtype=np.float32
        )
        t = TrajectoryBatch(**traj_data)
        del t
...
def test_env_infos_not_ndarray_traj(traj_data):
    with pytest.raises(ValueError, match="entry in env_infos must be a numpy array"):
...
def test_env_infos_batch_mismatch_traj(traj_data):
    with pytest.raises(
        ValueError, match="entry in env_infos must have a batch dimension"
    ):
...
def test_agent_infos_not_ndarray_traj(traj_data):
    with pytest.raises(ValueError, match="entry in agent_infos must be a numpy array"):
...
def test_agent_infos_batch_mismatch_traj(traj_data):
    with pytest.raises(
        ValueError, match="entry in agent_infos must have a batch dimension"
    ):
        traj_data["agent_infos"]["hidden"] = traj_data["agent_infos"]["hidden"][:-1]
        t = TrajectoryBatch(**traj_data)
        del t
...
def test_to_trajectory_list(traj_data):
    ...
    start = 0
    for length, last_obs, s in zip(
        traj_data["lengths"], traj_data["last_observations"], t_list
    ):
        stop = start + length
        assert (s["observations"] == traj_data["observations"][start:stop]).all()
        assert (
            s["next_observations"]
            == np.concatenate((traj_data["observations"][start + 1 : stop], [last_obs]))
        ).all()
...
def sample_data():
    # spaces
    obs_space = gym.spaces.Box(low=1, high=10, shape=(4, 3, 2), dtype=np.float32)
    act_space = gym.spaces.MultiDiscrete([2, 5])
    env_spec = EnvSpec(obs_space, act_space)
...
def test_new_time_step(sample_data):
    ...
    obs_space = akro.Box(low=-1000, high=1000, shape=(4, 3, 2), dtype=np.float32)
...
def test_obs_env_spec_mismatch_time_step(sample_data):
    with pytest.raises(
        ValueError, match="observation must conform to observation_space"
    ):
    ...
    with pytest.raises(
        ValueError, match="observation should have the same dimensionality"
    ):
        sample_data["observation"] = sample_data["observation"][:, :, :1]
        s = TimeStep(**sample_data)
        del s
...
def test_next_obs_env_spec_mismatch_time_step(sample_data):
    with pytest.raises(
        ValueError, match="next_observation must conform to observation_space"
    ):
        sample_data["next_observation"] = sample_data["next_observation"][:, :, :1]
        s = TimeStep(**sample_data)
        del s
    ...
    with pytest.raises(
        ValueError, match="next_observation should have the same dimensionality"
    ):
        sample_data["next_observation"] = sample_data["next_observation"][:, :, :1]
        s = TimeStep(**sample_data)
        del s
...
def test_act_env_spec_mismatch_time_step(sample_data):
    with pytest.raises(ValueError, match="action must conform to action_space"):
    ...
    with pytest.raises(ValueError, match="action should have the same dimensionality"):
        sample_data["action"] = sample_data["action"][:-1]
        s = TimeStep(**sample_data)
        del s
...
def test_observations_env_spec_mismatch_batch(batch_data):
    ...
    with pytest.raises(
        ValueError, match="observations should have the same dimensionality"
    ):
        batch_data["observations"] = batch_data["observations"][:, :, :, :1]
        s = TimeStepBatch(**batch_data)
        del s
...
def test_next_observations_env_spec_mismatch_batch(batch_data):
    ...
    with pytest.raises(
        ValueError, match="next_observations should have the same dimensionality"
    ):
        batch_data["next_observations"] = batch_data["next_observations"][:, :, :, :1]
        s = TimeStepBatch(**batch_data)
        del s
...
def test_next_observations_batch_mismatch_batch(batch_data):
    with pytest.raises(ValueError, match="batch dimension of " "next_observations"):
        batch_data["next_observations"] = batch_data["next_observations"][:-1]
        s = TimeStepBatch(**batch_data)
        del s
...
def test_act_box_env_spec_mismatch_batch(batch_data):
    with pytest.raises(ValueError, match="actions should have"):
        batch_data["env_spec"].action_space = akro.Box(
            low=1, high=np.inf, shape=(4, 3, 2), dtype=np.float32
        )
        s = TimeStepBatch(**batch_data)
        del s
...
def test_agent_infos_batch_mismatch_batch(batch_data):
    with pytest.raises(
        ...
        batch_data["agent_infos"]["hidden"] = batch_data["agent_infos"]["hidden"][:-1]
        s = TimeStepBatch(**batch_data)
        del s
...
def test_concatenate_batch(batch_data):
    ...
    new_obs = np.concatenate([batch_data["observations"], batch_data["observations"]])
    new_next_obs = np.concatenate(
        [batch_data["next_observations"], batch_data["next_observations"]]
    )
    new_actions = np.concatenate([batch_data["actions"], batch_data["actions"]])
    new_rewards = np.concatenate([batch_data["rewards"], batch_data["rewards"]])
    new_terminals = np.concatenate([batch_data["terminals"], batch_data["terminals"]])
    new_env_infos = {
...
def test_split_batch(batch_data):
    ...
    for i, batch in enumerate(batches):
        ...
        assert np.array_equal(batch.observations, [batch_data["observations"][i]])
        assert np.array_equal(
            batch.next_observations, [batch_data["next_observations"][i]]
        )
        ...
        for key in batch.env_infos:
            ...
            assert np.array_equal(
                batch.env_infos[key], [batch_data["env_infos"][key][i]]
            )
        for key in batch.agent_infos:
            assert key in batch_data["agent_infos"]
            assert np.array_equal(
                batch.agent_infos[key], [batch_data["agent_infos"][key][i]]
            )
