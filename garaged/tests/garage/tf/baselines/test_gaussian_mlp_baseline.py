...
class TestGaussianMLPBaseline(TfGraphTestCase):
    ...
    def test_fit(self, obs_dim):
        ...
        with mock.patch(
            ("garage.tf.baselines." "gaussian_mlp_baseline." "GaussianMLPRegressor"),
            new=SimpleGaussianMLPRegressor,
        ):
            gmb = GaussianMLPBaseline(env_spec=box_env.spec)
        paths = [
            {"observations": [np.full(obs_dim, 1)], "returns": [1]},
            {"observations": [np.full(obs_dim, 2)], "returns": [2]},
        ]
        gmb.fit(paths)
    ...
    def test_param_values(self, obs_dim):
        ...
        with tf.compat.v1.variable_scope("GaussianMLPBaseline", reuse=True):
            return_var = tf.compat.v1.get_variable("SimpleGaussianMLPModel/return_var")
        return_var.load(1.0)
    ...
    def test_get_params_internal(self, obs_dim):
        ...
        with mock.patch(
            ...
            gmb = GaussianMLPBaseline(env_spec=box_env.spec, regressor_args=dict())
        params_interal = gmb.get_params_internal()
        trainable_params = tf.compat.v1.trainable_variables(scope="GaussianMLPBaseline")
