...
class TestGaussianMLPEncoder(TfGraphTestCase):
    ...
    def test_get_embedding(self, obs_dim, embedding_dim):
        ...
        embedding_spec = InOutSpec(
            input_space=env.spec.observation_space, output_space=env.spec.action_space
        )
        embedding = GaussianMLPEncoder(embedding_spec)
        task_input = tf.compat.v1.placeholder(
            tf.float32, shape=(None, None, embedding.input_dim)
        )
    ...
    def test_is_pickleable(self, obs_dim, embedding_dim):
        ...
        with tf.compat.v1.variable_scope(
            "GaussianMLPEncoder/GaussianMLPModel", reuse=True
        ):
            bias = tf.compat.v1.get_variable("dist_params/mean_network/hidden_0/bias")
        # assign it to all one
        bias.load(tf.ones_like(bias).eval())
        output1 = self.sess.run(
            [embedding.distribution.loc, embedding.distribution.stddev()],
    ...
    def test_auxiliary(self):
        ...
        assert (
            embedding.distribution.loc.get_shape().as_list()[-1]
        ...
        assert sorted(
            map(np.shape, embedding.flat_to_params(np.ones(var_count)))
        ) == sorted(var_shapes)
