...
class TestCategoricalMLPModel(TfGraphTestCase):
    ...
    def setup_method(self):
        ...
        input_width = 10
        input_height = 10
        self._obs_input = np.ones((batch_size, 1, input_width, input_height, 3))
        self._input_shape = (input_width, input_height, 3)  # height, width, channel
        self._input_ph = tf.compat.v1.placeholder(
            tf.float32, shape=(None, None) + self._input_shape, name="input"
        )

    def test_dist(self):
        model = CategoricalCNNModel(
            output_dim=1, filters=((5, (3, 3)),), strides=(1,), padding="VALID"
        )
    ...
    # yapf: enable
    def test_is_pickleable(self, output_dim, filters, strides, padding, hidden_sizes):
        model = CategoricalCNNModel(
        ...
        output1 = self.sess.run(dist.probs, feed_dict={self._input_ph: self._obs_input})

        h = pickle.dumps(model)
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            input_var = tf.compat.v1.placeholder(
                tf.float32, shape=(None, None) + self._input_shape
            )
            model_pickled = pickle.loads(h)
            dist2 = model_pickled.build(input_var).dist
            output2 = sess.run(dist2.probs, feed_dict={input_var: self._obs_input})
