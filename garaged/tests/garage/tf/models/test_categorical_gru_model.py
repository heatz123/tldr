...
class TestCategoricalGRUModel(TfGraphTestCase):
    ...
    def test_dist(self):
        ...
        dist = model.build(self.input_var, self.step_input_var, step_hidden_var).dist
        assert isinstance(dist, tfp.distributions.OneHotCategorical)

    def test_output_nonlinearity(self):
        model = CategoricalGRUModel(
    ...
    def test_is_pickleable(self):
        ...
        network = model.build(self.input_var, self.step_input_var, step_hidden_var)
        dist = network.dist
        # assign bias to all one
        with tf.compat.v1.variable_scope("CategoricalGRUModel/gru", reuse=True):
        ...
        outputs1 = self.sess.run(
            dist.probs, feed_dict={self.input_var: self.obs_inputs}
        )
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            network2 = model_pickled.build(input_var, step_input_var, step_hidden_var)
            dist2 = network2.dist
