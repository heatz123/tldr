...
class TestCategoricalLSTMModel(TfGraphTestCase):
    ...
    def setup_method(self):
        ...
        self.obs_inputs = np.full(
            (self.batch_size, self.time_step, self.feature_shape), 1.0
        )
        self.obs_input = np.full((self.batch_size, self.feature_shape), 1.0)
    ...
    def test_dist(self):
        ...
        dist = model.build(
            self._input_var, self._step_input_var, step_hidden_var, step_cell_var
        ).dist
        assert isinstance(dist, tfp.distributions.OneHotCategorical)

    def test_output_nonlinearity(self):
        model = CategoricalLSTMModel(
            output_dim=1, hidden_dim=4, output_nonlinearity=lambda x: x / 2
        )
        ...
        obs = np.ones((1, 1, 1))
        dist = model.build(obs_ph, step_obs_ph, step_hidden_ph, step_cell_ph).dist
        probs = tf.compat.v1.get_default_session().run(
            dist.probs, feed_dict={obs_ph: obs}
        )
        assert probs == [0.5]
    ...
    def test_output_normalized(self, output_dim):
        ...
        obs_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, None, output_dim))
        step_obs_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, output_dim))
        step_hidden_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, 4))
        step_cell_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, 4))
        obs = np.ones((1, 1, output_dim))
        dist = model.build(obs_ph, step_obs_ph, step_hidden_ph, step_cell_ph).dist
        probs = tf.compat.v1.get_default_session().run(
            tf.reduce_sum(dist.probs), feed_dict={obs_ph: obs}
        )
    ...
    def test_is_pickleable(self):
        ...
        with tf.compat.v1.variable_scope("CategoricalLSTMModel/lstm", reuse=True):
        ...
        outputs1 = self.sess.run(
            dist.probs, feed_dict={self._input_var: self.obs_inputs}
        )
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            step_hidden_var = tf.compat.v1.placeholder(
                shape=(self.batch_size, 1), name="initial_hidden", dtype=tf.float32
            )
            step_cell_var = tf.compat.v1.placeholder(
                shape=(self.batch_size, 1), name="initial_cell", dtype=tf.float32
            )

            network2 = model_pickled.build(
                input_var, step_input_var, step_hidden_var, step_cell_var
            )
            dist2 = network2.dist
            outputs2 = sess.run(dist2.probs, feed_dict={input_var: self.obs_inputs})
            output2 = sess.run(
                [network2.step_output, network2.step_hidden, network2.step_cell],
                feed_dict={
