...
class TestCategoricalMLPModel(TfGraphTestCase):
    ...
    def test_output_nonlinearity(self):
        model = CategoricalMLPModel(output_dim=1, output_nonlinearity=lambda x: x / 2)
    ...
    def test_is_pickleable(self, output_dim, hidden_sizes):
        ...
        with tf.compat.v1.variable_scope("CategoricalMLPModel/mlp", reuse=True):
            bias = tf.compat.v1.get_variable("hidden_0/bias")

        bias.load(tf.ones_like(bias).eval())

        output1 = self.sess.run(dist.probs, feed_dict={self.input_var: self.obs})
