...
class TestCNNMLPMergeModel(TfGraphTestCase):
    ...
    def setup_method(self):
        ...
        self.obs_ph = tf.compat.v1.placeholder(
            tf.float32, shape=(None,) + self.input_shape, name="input"
        )
        self.action_input = np.ones((self.batch_size, 3))

        self.action_ph = tf.compat.v1.placeholder(
            tf.float32, shape=(None,) + self.action_input.shape[1:], name="input"
        )
        self.action_input = np.ones((self.batch_size, 3))
    ...
    def test_output_value(self, filters, in_channels, strides, hidden_sizes):
        ...
        # flatten
        cnn_output = np.full(
            (self.batch_size, flatten_shape), filter_sum, dtype=np.float32
        )

        with tf.compat.v1.variable_scope(
            "cnn_mlp_merge_model1/MLPMergeModel", reuse=True
        ):
        ...
        mlp_output = self.sess.run(
            model_out,
            feed_dict={self.obs_ph: self.obs_input, self.action_ph: self.action_input},
        )
        ...
        # output
        h1_in = tf.matmul(tf.concat([h0_out, self.action_input], 1), out_w) + out_b
    ...
    def test_output_value_max_pooling(
        self, filters, in_channels, strides, pool_strides, pool_shapes
    ):
        model = CNNMLPMergeModel(
            filters=filters,
            strides=strides,
        ...
        for filter_iter, stride in zip(filters, strides):
            height_size = int((height_size - filter_iter[1][0]) / stride) + 1
            height_size = int((height_size - pool_shapes[0]) / pool_strides[0]) + 1
            width_size = int((width_size - filter_iter[1][1]) / stride) + 1
            width_size = int((width_size - pool_shapes[1]) / pool_strides[1]) + 1
        ...
        with tf.compat.v1.variable_scope(
            "cnn_mlp_merge_model2/MLPMergeModel", reuse=True
        ):
            h0_w = tf.compat.v1.get_variable("mlp_concat/hidden_0/kernel")
            h0_b = tf.compat.v1.get_variable("mlp_concat/hidden_0/bias")
            out_w = tf.compat.v1.get_variable("mlp_concat/output/kernel")
    ...
    def test_is_pickleable(self, filters, strides):
        ...
        output1 = self.sess.run(outputs, feed_dict={self.obs_ph: self.obs_input})
