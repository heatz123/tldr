...
class TestGaussianCNNModel(TfGraphTestCase):
    ...
    def setup_method(self):
        ...
        self.obs = np.full((self.batch_size, self.input_width, self.input_height, 3), 1)
    ...
    @pytest.mark.parametrize(
        "filters, in_channels, strides, output_dim, " "hidden_sizes",
        [
            (((3, (1, 1)),), (3,), (1,), 1, (1,)),
            (((3, (3, 3)),), (3,), (1,), 2, (2,)),
            (((3, (3, 3)),), (3,), (2,), 1, (1, 1)),
            (((3, (1, 1)), (3, (1, 1))), (3, 3), (1, 1), 2, (2, 2)),
            (((3, (3, 3)), (3, (3, 3))), (3, 3), (2, 2), 2, (2, 2)),
        ],
    )
    def test_std_share_network_output_values(
        self, mock_normal, filters, in_channels, strides, output_dim, hidden_sizes
    ):
        ...
        expected_mean = np.full(
            (self.batch_size, output_dim), network_output, dtype=np.float32
        )
        expected_std_param = np.full(
            (self.batch_size, output_dim), network_output, dtype=np.float32
        )
        expected_log_std = np.full(
            (self.batch_size, output_dim), network_output, dtype=np.float32
        )
    ...
    def test_without_std_share_network_output_values(
        ...
        expected_std_param = np.full(
            (self.batch_size, output_dim), np.log(2), dtype=np.float32
        )
        expected_log_std = np.full(
            (self.batch_size, output_dim), np.log(2), dtype=np.float32
        )
    ...
    @pytest.mark.parametrize(
        "output_dim, hidden_sizes, std_hidden_sizes",
        [
            (1, (0,), (0,)),
            (1, (1,), (1,)),
            (1, (2,), (2,)),
    ...
    def test_adaptive_std_output_shape(
        self, output_dim, hidden_sizes, std_hidden_sizes
    ):
        model = GaussianCNNModel(
            filters=((3, (3, 3)), (6, (3, 3))),
            strides=[1, 1],
    ...
    @mock.patch("tensorflow.random.normal")
    def test_std_share_network_is_pickleable(
        self, mock_normal, output_dim, hidden_sizes
    ):
        mock_normal.return_value = 0.5
        input_var = tf.compat.v1.placeholder(tf.float32, shape=(None, 10, 10, 3))
        model = GaussianCNNModel(
            filters=((3, (3, 3)), (6, (3, 3))),
            strides=[1, 1],
            padding="SAME",
        ...
        with tf.compat.v1.variable_scope("GaussianCNNModel", reuse=True):
            bias = tf.compat.v1.get_variable("dist_params/mean_std_network/output/bias")
    ...
    def test_without_std_share_network_is_pickleable(
        ...
        # get output bias
        with tf.compat.v1.variable_scope("GaussianCNNModel", reuse=True):
            bias = tf.compat.v1.get_variable("dist_params/mean_network/output/bias")
        # assign it to all ones
        bias.load(tf.ones_like(bias).eval())
    ...
    @mock.patch("tensorflow.random.normal")
    def test_adaptive_std_is_pickleable(
    ...
    @mock.patch("tensorflow.random.normal")
    def test_softplus_output_values(self, mock_normal, output_dim, hidden_sizes):
        mock_normal.return_value = 0.5
        filters = ((3, (3, 3)), (6, (3, 3)))
        in_channels = [3, 3]
        strides = [1, 1]
        ...
        expected_std_param = np.full(
            (self.batch_size, output_dim), np.log(np.exp(2) - 1), dtype=np.float32
        )
        expected_log_std = np.log(np.log(1.0 + np.exp(expected_std_param)))
