...
class TestGaussianLSTMModel(TfGraphTestCase):
    ...
    def test_dist(self):
        ...
        dist = model.build(
            self.input_var, self.step_input_var, step_hidden_var, step_cell_var
        ).dist
    ...
    def test_std_share_network_shapes(self, output_dim, hidden_dim):
        ...
        model.build(self.input_var, self.step_input_var, step_hidden_var, step_cell_var)
    ...
    @pytest.mark.parametrize(
        "output_dim, hidden_dim, init_std",
        [(1, 1, 1), (1, 1, 2), (1, 2, 1), (1, 2, 2), (3, 3, 1), (3, 3, 2)],
    )
    @mock.patch("tensorflow.random.normal")
    def test_without_std_share_network_output_values(
        ...
        hidden1 = hidden2 = np.full(
            (self.batch_size, hidden_dim), hidden_init_var.eval()
        )
        cell1 = cell2 = np.full((self.batch_size, hidden_dim), cell_init_var.eval())
        ...
        for _ in range(self.time_step):
            ...
            output_nonlinearity = np.full((np.prod(hidden2.shape[1:]), output_dim), 0.1)
            output2 = np.matmul(hidden2, output_nonlinearity)
            assert np.allclose(mean1, output2)
            expected_log_std = np.full((self.batch_size, output_dim), np.log(init_std))
