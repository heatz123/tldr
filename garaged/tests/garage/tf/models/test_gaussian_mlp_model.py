...
class TestGaussianMLPModel(TfGraphTestCase):
    ...
    def setup_method(self):
        super().setup_method()
        self.input_var = tf.compat.v1.placeholder(tf.float32, shape=(None, None, 5))
    ...
    def test_std_share_network_output_values(self, output_dim, hidden_sizes):
        ...
        mean, log_std = self.sess.run(
            [dist.loc, tf.math.log(dist.stddev())], feed_dict={self.input_var: self.obs}
        )

        expected_mean = np.full([1, 1, output_dim], 5 * np.prod(hidden_sizes))
        expected_log_std = np.full([1, 1, output_dim], 5 * np.prod(hidden_sizes))
    ...
    def test_std_share_network_shapes(self, output_dim, hidden_sizes):
        ...
        model = GaussianMLPModel(
            output_dim=output_dim, hidden_sizes=hidden_sizes, std_share_network=True
        )
    ...
    def test_without_std_share_network_output_values(self, output_dim, hidden_sizes):
        model = GaussianMLPModel(
            output_dim=output_dim,
            hidden_sizes=hidden_sizes,
            init_std=2,
        ...
        expected_mean = np.full([1, 1, output_dim], 5 * np.prod(hidden_sizes))
        expected_log_std = np.full([1, 1, output_dim], np.log(2.0))
        assert np.array_equal(mean, expected_mean)
    ...
    def test_adaptive_std_network_output_values(
        ...
        expected_mean = np.full([1, 1, output_dim], 5 * np.prod(hidden_sizes))
        expected_log_std = np.full([1, 1, output_dim], 5 * np.prod(std_hidden_sizes))
        assert np.array_equal(mean, expected_mean)
        assert np.array_equal(log_std, expected_log_std)
    ...
    def test_without_std_share_network_is_pickleable(self, output_dim, hidden_sizes):
        model = GaussianMLPModel(
            output_dim=output_dim,
            hidden_sizes=hidden_sizes,
            std_share_network=False,
    ...
    def test_exp_min_std(self, output_dim, hidden_sizes):
        ...
        log_std = self.sess.run(
            tf.math.log(dist.stddev()), feed_dict={self.input_var: self.obs}
        )
