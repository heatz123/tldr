...
class TestGRU(TfGraphTestCase):
    ...
    def test_output_shapes(self, time_step, input_dim, output_dim, hidden_init):
        obs_inputs = np.full((self.batch_size, time_step, input_dim), 1.0)
        obs_input = np.full((self.batch_size, input_dim), 1.0)
        ...
        hidden = np.full((self.batch_size, self.hidden_dim), hidden_init.eval())

        for _ in range(time_step):
            output, hidden = self.sess.run(
                [output_t, h_t],
                feed_dict={
        ...
        full_output = self.sess.run(outputs_t, feed_dict={input_var: obs_inputs})
    ...
    def test_output_value(self, time_step, input_dim, output_dim, hidden_init):
        ...
        # Compute output by doing t step() on the gru cell
        outputs_t, output_t, h_t, hidden_init = self.gru
        hidden1 = hidden2 = np.full(
            (self.batch_size, self.hidden_dim), hidden_init.eval()
        )

        for i in range(time_step):
            output1, hidden1 = self.sess.run(
                [output_t, h_t],
                feed_dict={step_input_var: obs_input, self.step_hidden_var: hidden1},
            )  # noqa: E126
            ...
            hidden2 = recurrent_step_gru(
                ...
                gate_nonlinearity=lambda x: 1.0 / (1.0 + np.exp(-x)),
            )

            output_nonlinearity = np.full((np.prod(hidden2.shape[1:]), output_dim), 1.0)
            output2 = np.matmul(hidden2, output_nonlinearity)

            assert np.allclose(output1, output2)
            assert np.allclose(hidden1, hidden2)

        full_output1 = self.sess.run(outputs_t, feed_dict={input_var: obs_inputs})

        hidden2 = np.full((self.batch_size, self.hidden_dim), hidden_init.eval())
        stack_hidden = None
        for i in range(time_step):
            hidden2 = recurrent_step_gru(
    ...
    # yapf: enable
    def test_output_value_trainable_hidden_and_cell(
        self, time_step, input_dim, output_dim
    ):
        obs_inputs = np.full((self.batch_size, time_step, input_dim), 1.0)
        obs_input = np.full((self.batch_size, input_dim), 1.0)
    ...
    def test_gradient_paths(self):
        ...
        self.sess.run(
            [grads_step_o_i, grads_step_o_h, grads_step_h],
            feed_dict={
                step_input_var: obs_input,
                self.step_hidden_var: hidden,
            },
    ...
    # yapf: enable
    def test_output_same_as_rnn(self, time_step, input_dim, output_dim, hidden_init):
        obs_inputs = np.full((self.batch_size, time_step, input_dim), 1.0)
        obs_input = np.full((self.batch_size, input_dim), 1.0)
        ...
        outputs, hiddens = self.sess.run(
            [outputs, hiddens], feed_dict={input_var: obs_inputs}
        )
        ...
        full_outputs = self.sess.run(self.gru[0], feed_dict={input_var: obs_inputs})
