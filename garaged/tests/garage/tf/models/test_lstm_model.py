...
class TestLSTMModel(TfGraphTestCase):
    ...
    @pytest.mark.parametrize("output_dim, hidden_dim", [(1, 1), (1, 2), (3, 3)])
    def test_output_values(self, output_dim, hidden_dim):
        ...
        step_hidden_var = tf.compat.v1.placeholder(
            shape=(self.batch_size, hidden_dim), name="step_hidden", dtype=tf.float32
        )
        step_cell_var = tf.compat.v1.placeholder(
            shape=(self.batch_size, hidden_dim), name="step_cell", dtype=tf.float32
        )

        outputs = model.build(
            self._input_var, self._step_input_var, step_hidden_var, step_cell_var
        ).outputs
        output = self.sess.run(outputs[0], feed_dict={self._input_var: self.obs_inputs})
    ...
    def test_is_pickleable(self):
        ...
        outputs1 = self.sess.run(
            network.all_output, feed_dict={self._input_var: self.obs_inputs}
        )
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            outputs2 = sess.run(
                network2.all_output, feed_dict={input_var: self.obs_inputs}
            )
            output2 = sess.run(
