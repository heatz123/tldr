...
class TestModel(TfGraphTestCase):
    ...
    def test_same_model_with_no_name(self):
        input_var = tf.compat.v1.placeholder(tf.float32, shape=(None, 5))
        another_input_var = tf.compat.v1.placeholder(tf.float32, shape=(None, 5))
        model = SimpleModel(output_dim=2)
    ...
    def test_model_with_different_name(self):
        ...
        data = np.ones((3, 5))
        results_1, results_2 = self.sess.run(
            [outputs_1, outputs_2], feed_dict={input_var: data, another_input_var: data}
        )  # noqa: E126
    ...
    def test_complicated_model_is_pickleable(self):
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            results2 = sess.run(
                model_pickled._networks["default"].outputs, feed_dict={input_var: data}
            )
    ...
    def test_simple_model_is_pickleable_with_same_parameters(self):
        ...
        with tf.compat.v1.Session(graph=tf.Graph()):
            state = tf.compat.v1.placeholder(shape=[None, 10, 5], dtype=tf.float32)
            model.build(state)

            model.parameters = {
                k: np.zeros_like(v) for k, v in model.parameters.items()
            }
    ...
    def test_simple_model_is_pickleable_with_missing_parameters(self):
        ...
        with tf.compat.v1.Session(graph=tf.Graph()):
            ...
            del model_pickled._default_parameters["SimpleModel/state/hidden_0/kernel:0"]
    ...
    def test_model_set_parameters(self):
        ...
        for m1, m2 in zip(
            collections.OrderedDict(sorted(model1.parameters.items())).values(),
            collections.OrderedDict(sorted(model2.parameters.items())).values(),
        ):
