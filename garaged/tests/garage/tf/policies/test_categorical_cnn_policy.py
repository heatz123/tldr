...
class TestCategoricalCNNPolicyWithModel(TfGraphTestCase):
    ...
    @pytest.mark.parametrize(
        "filters, strides, padding, hidden_sizes",
        [
            (((3, (3, 3)),), (1,), "VALID", (4,)),
    ...
    def test_build(self, filters, strides, padding, hidden_sizes):
        ...
        output1 = self.sess.run(
            [policy.distribution.probs], feed_dict={policy.model.input: [[obs]]}
        )
        output2 = self.sess.run([dist_sym.probs], feed_dict={state_input: [[obs]]})
    ...
    def test_does_not_support_dict_obs_space(
        self, filters, strides, padding, hidden_sizes
    ):
...
class TestCategoricalCNNPolicyImageObs(TfGraphTestCase):
    ...
    def test_obs_unflattened(self, filters, strides, padding, hidden_sizes):
        self.policy = CategoricalCNNPolicy(
