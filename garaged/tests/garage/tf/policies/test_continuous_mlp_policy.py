...
class TestContinuousMLPPolicy(TfGraphTestCase):
    ...
    def test_get_action(self, obs_dim, action_dim, obs_type):
        ...
        with mock.patch(
            ("garage.tf.policies." "continuous_mlp_policy.MLPModel"), new=SimpleMLPModel
        ):
    ...
    def test_get_action_sym(self, obs_dim, action_dim):
        ...
        action = self.sess.run(action_sym, feed_dict={state_input: [obs.flatten()]})
    ...
    def test_is_pickleable(self, obs_dim, action_dim):
        ...
        with tf.compat.v1.variable_scope("ContinuousMLPPolicy/MLPModel", reuse=True):
            return_var = tf.compat.v1.get_variable("return_var")
        # assign it to all one
        return_var.load(tf.ones_like(return_var).eval())
        output1 = self.sess.run(
            policy.model.outputs, feed_dict={policy.model.input: [obs.flatten()]}
        )
