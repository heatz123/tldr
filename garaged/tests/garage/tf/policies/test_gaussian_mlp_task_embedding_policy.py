...
class TestGaussianMLPTaskEmbeddingPolicy(TfGraphTestCase):
    ...
    def test_get_action(self, obs_dim, task_num, latent_dim, action_dim):
        ...
        embedding_spec = InOutSpec(
            input_space=akro.Box(low=np.zeros(task_num), high=np.ones(task_num)),
            output_space=akro.Box(low=np.zeros(latent_dim), high=np.ones(latent_dim)),
        )
    ...
    def test_auxiliary(self):
        ...
        assert (
            policy.distribution.loc.get_shape().as_list()[-1]
        ...
        assert (
            policy.augmented_observation_space.flat_dim
    ...
    def test_get_vars(self):
        ...
        policy = GaussianMLPTaskEmbeddingPolicy(
            env_spec=env.spec, encoder=encoder, hidden_sizes=[32, 32, 32]
        )
    ...
    def test_does_not_support_non_box_obs_space(self):
        ...
        with pytest.raises(
            ValueError,
            match=(
                "This task embedding policy does not support"
                "non akro.Box observation spaces."
            ),
        ):
            embedding_spec = InOutSpec(
                input_space=akro.Box(low=np.zeros(task_num), high=np.ones(task_num)),
                output_space=akro.Box(
                    low=np.zeros(latent_dim), high=np.ones(latent_dim)
                ),
            )
            encoder = GaussianMLPEncoder(embedding_spec, hidden_sizes=[32, 32, 32])
