...
class TestQfDerivedPolicy(TfGraphTestCase):
    ...
    def setup_method(self):
        ...
        self.policy = DiscreteQfDerivedPolicy(env_spec=self.env.spec, qf=self.qf)
    ...
    def test_is_pickleable(self):
        with tf.compat.v1.variable_scope("SimpleQFunction/SimpleMLPModel", reuse=True):
    ...
    def test_does_not_support_dict_obs_space(self):
        ...
        with pytest.raises(ValueError):
            qf = SimpleQFunction(env.spec, name="does_not_support_dict_obs_space")
...
class TestQfDerivedPolicyImageObs(TfGraphTestCase):
    ...
    def setup_method(self):
        super().setup_method()
        self.env = GarageEnv(AtariEnv(DummyDiscretePixelEnvBaselines()), is_image=True)
        self.qf = DiscreteCNNQFunction(
            env_spec=self.env.spec, filters=((1, (1, 1)),), strides=(1,), dueling=False
        )
        self.policy = DiscreteQfDerivedPolicy(env_spec=self.env.spec, qf=self.qf)
        self.sess.run(tf.compat.v1.global_variables_initializer())
        self.env.reset()
