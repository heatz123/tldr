...
class TestContinuousCNNQFunction(TfGraphTestCase):
    ...
    def test_get_qval(self, filters, strides):
        ...
        with mock.patch(
            ("garage.tf.models." "cnn_mlp_merge_model.CNNModel"), new=SimpleCNNModel
        ):
            with mock.patch(
                ("garage.tf.models." "cnn_mlp_merge_model.MLPMergeModel"),
                new=SimpleMLPMergeModel,
            ):
                qf = ContinuousCNNQFunction(
                    env_spec=env.spec, filters=filters, strides=strides
                )
    ...
    # yapf: enable
    def test_get_qval_max_pooling(self, filters, strides, pool_strides, pool_shapes):
        env = GarageEnv(DummyDiscretePixelEnv())
        obs = env.reset()

        with mock.patch(
            ("garage.tf.models." "cnn_mlp_merge_model.CNNModelWithMaxPooling"),
    ...
    def test_invalid_obs_dim(self, obs_dim):
        with pytest.raises(ValueError):
            ...
            ContinuousCNNQFunction(
                env_spec=env.spec, filters=((5, (3, 3)),), strides=(1,)
            )
    ...
    def test_not_box(self):
        with pytest.raises(ValueError):
            ...
            ContinuousCNNQFunction(
                env_spec=dict_env.spec, filters=((5, (3, 3)),), strides=(1,)
            )

    def test_obs_is_image(self):
        image_env = GarageEnv(DummyDiscretePixelEnv(), is_image=True)
        with mock.patch(
            ("tests.fixtures.models.SimpleCNNModel._build"),
            autospec=True,
            side_effect=SimpleCNNModel._build,
        ) as build:
            with mock.patch(
                ("garage.tf.models." "cnn_mlp_merge_model.CNNModel"), new=SimpleCNNModel
            ):
                with mock.patch(
                    ...
                    qf = ContinuousCNNQFunction(
                        env_spec=image_env.spec, filters=((5, (3, 3)),), strides=(1,)
                    )
                    ...
                    assert (
                        self.sess.run(
                            normalized_obs, feed_dict={qf.inputs[0]: fake_obs}
                        )
                        == 1.0
                    ).all()
                    ...
                    action_input = tf.compat.v1.placeholder(
                        tf.float32, shape=(None,) + act_dim
                    )
                    ...
                    assert (
                        self.sess.run(normalized_obs, feed_dict={state_input: fake_obs})
                        == 1.0
                    ).all()
        ...
        with mock.patch(
            ...
            with mock.patch(
                ...
                with mock.patch(
                    ...
                    fake_obs = [np.full(env.spec.observation_space.shape, 255.0)]
    ...
    def test_get_qval_sym(self, filters, strides):
        ...
        input_var1 = tf.compat.v1.placeholder(tf.float32, shape=(None,) + obs.shape)
        input_var2 = tf.compat.v1.placeholder(tf.float32, shape=(None,) + act.shape)
    ...
    def test_is_pickleable(self, filters, strides):
        ...
        with tf.compat.v1.variable_scope(
            "ContinuousCNNQFunction/CNNMLPMergeModel/SimpleMLPMergeModel", reuse=True
        ):
    ...
    def test_clone(self, filters, strides):
        ...
        for cloned_param, param in zip(
            qf_clone.model.parameters.values(), qf.model.parameters.values()
        ):
