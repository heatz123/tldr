...
class TestDiscreteCNNQFunction(TfGraphTestCase):
    ...
    def test_get_action(self, filters, strides):
        with mock.patch(
            ("garage.tf.q_functions." "discrete_cnn_q_function.CNNModel"),
            new=SimpleCNNModel,
        ):
            with mock.patch(
                ("garage.tf.q_functions." "discrete_cnn_q_function.MLPModel"),
                new=SimpleMLPModel,
            ):
    ...
    def test_obs_is_image(self):
        ...
        with mock.patch(
            ("garage.tf.models." "categorical_cnn_model.CNNModel._build"),
            autospec=True,
            side_effect=CNNModel._build,
        ) as build:

            qf = DiscreteCNNQFunction(
                env_spec=image_env.spec,
                filters=((5, (3, 3)),),
                strides=(2,),
                dueling=False,
            )
            ...
            assert (
                self.sess.run(normalized_obs, feed_dict={input_ph: fake_obs}) == 1.0
            ).all()
            ...
            fake_obs = [np.full(image_env.spec.observation_space.shape, 255)]
            assert (
                self.sess.run(normalized_obs, feed_dict={state_input: fake_obs}) == 1.0
            ).all()
    ...
    def test_obs_not_image(self):
        ...
        with mock.patch(
            ...
            qf = DiscreteCNNQFunction(
                env_spec=env.spec, filters=((5, (3, 3)),), strides=(2,), dueling=False
            )
            normalized_obs = build.call_args_list[0][0][1]
            ...
            assert (
                self.sess.run(normalized_obs, feed_dict={input_ph: fake_obs}) == 255.0
            ).all()
            ...
            fake_obs = [np.full(env.spec.observation_space.shape, 255)]
            assert (
                self.sess.run(normalized_obs, feed_dict={state_input: fake_obs}) == 255
            ).all()
    ...
    def test_get_action_dueling(self, filters, strides):
        with mock.patch(
            ...
            with mock.patch(
                ("garage.tf.q_functions." "discrete_cnn_q_function.MLPDuelingModel"),
                new=SimpleMLPModel,
            ):
                qf = DiscreteCNNQFunction(
                    env_spec=self.env.spec,
    ...
    def test_get_action_max_pooling(self, filters, strides, pool_strides, pool_shapes):
        with mock.patch(
            ("garage.tf.q_functions." "discrete_cnn_q_function.CNNModelWithMaxPooling"),
