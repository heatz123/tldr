...
class TestDiscreteMLPQFunction(TfGraphTestCase):
    ...
    def test_get_action(self, obs_dim, action_dim):
        ...
        with mock.patch(
            ("garage.tf.q_functions." "discrete_mlp_q_function.MLPModel"),
            new=SimpleMLPModel,
        ):
        ...
        outputs = self.sess.run(qf.q_vals, feed_dict={qf.input: [obs, obs, obs]})
    ...
    def test_output_shape_dueling(self, obs_dim, action_dim):
        env = GarageEnv(DummyDiscreteEnv(obs_dim=obs_dim, action_dim=action_dim))
        with mock.patch(
            ("garage.tf.q_functions." "discrete_mlp_q_function.MLPDuelingModel"),
            new=SimpleMLPModel,
        ):
    ...
    def test_is_pickleable(self, obs_dim, action_dim):
        ...
        with tf.compat.v1.Session(graph=tf.Graph()) as sess:
            ...
            output2 = sess.run(qf_pickled.q_vals, feed_dict={qf_pickled.input: [obs]})
