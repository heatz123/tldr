...
def get_train_data(input_shape, output_dim):
    if input_shape == (1,):
        ...
        obs = [
            {"observations": [[x]], "returns": [get_labels(input_shape, x, output_dim)]}
            for x in data
        ]
    elif input_shape == (2,):
...
class TestBernoulliMLPRegressor(TfGraphTestCase):
    ...
    def test_fit_normalized(self, input_shape, output_dim):
        bmr = BernoulliMLPRegressor(input_shape=input_shape, output_dim=output_dim)
    ...
    def test_sample_predict(self):
        ...
        input_dim = 50
        output_dim = 1
        bmr = BernoulliMLPRegressor(input_shape=(input_dim,), output_dim=output_dim)
    ...
    def test_log_likelihood_sym(self, output_dim, input_shape):
        ...
        ll_from_sym = self.sess.run(
            outputs, feed_dict={new_xs_var: data, new_ys_var: one_hot_label}
        )

        assert np.allclose(ll, ll_from_sym, rtol=0, atol=1e-5)

    @mock.patch(
        "tests.garage.tf.regressors." "test_bernoulli_mlp_regressor." "LbfgsOptimizer"
    )
    @mock.patch(
