...
def get_train_data(input_shape):
    if input_shape == (1,):
        ...
        obs = [
            {"observations": [[x]], "returns": [get_labels(input_shape, x)]}
            for x in data
        ]
        ...
        obs = [
            {"observations": [x], "returns": [get_labels(input_shape, x)]} for x in data
        ]
    return obs
...
class TestCategoricalMLPRegressor(TfGraphTestCase):
    ...
    @pytest.mark.parametrize("input_shape, output_dim", [((1,), 2), ((2,), 2)])
    def test_fit_normalized(self, input_shape, output_dim):
        cmr = CategoricalMLPRegressor(input_shape=input_shape, output_dim=output_dim)
    ...
    def test_fit_unnormalized(self, input_shape, output_dim):
        cmr = CategoricalMLPRegressor(
            input_shape=input_shape, output_dim=output_dim, normalize_inputs=False
        )
        obs = get_train_data(input_shape)
    ...
    def test_fit_without_initial_trust_region(self, input_shape, output_dim):
        cmr = CategoricalMLPRegressor(
            input_shape=input_shape, output_dim=output_dim, use_trust_region=False
        )
        obs = get_train_data(input_shape)
    ...
    @mock.patch(
        "tests.garage.tf.regressors." "test_categorical_mlp_regressor." "LbfgsOptimizer"
    )
    @mock.patch(
        "tests.garage.tf.regressors."
    ...
    def test_is_pickleable(self):
        ...
        with tf.compat.v1.variable_scope(
            "CategoricalMLPRegressor/CategoricalMLPRegressorModel", reuse=True
        ):
