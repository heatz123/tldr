...
def get_train_test_data():
    matrices = [
        np.linspace(i - 0.5, i + 0.5, 300).reshape((10, 10, 3)) for i in range(110)
    ]
...
class TestGaussianCNNRegressor(TfGraphTestCase):
    ...
    def test_log_likelihood_sym(self, output_dim):
        ...
        outputs = gcr.log_likelihood_sym(new_input_var, new_ys_var, name="ll_sym")
        ll_from_sym = self.sess.run(
            outputs, feed_dict={new_input_var: [data], new_ys_var: [label]}
        )
    ...
    @mock.patch(
        "tests.garage.tf.regressors." "test_gaussian_cnn_regressor." "LbfgsOptimizer"
    )
