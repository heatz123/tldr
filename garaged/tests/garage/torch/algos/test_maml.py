...
try:
    ...
    pytest.skip(
        "To use mujoco-based features, please install garage[mujoco].",
...
class TestMAML:
    ...
    def setup_method(self):
        ...
        self.env = GarageEnv(normalize(HalfCheetahDirEnv(), expected_action_scale=10.0))
        ...
        self.value_function = GaussianMLPValueFunction(
            env_spec=self.env.spec, hidden_sizes=(32, 32)
        )
        self.algo = MAMLPPO(
            env=self.env,
            policy=self.policy,
    ...
    def test_adapt_policy(self):
        ...
        sampler = LocalSampler.from_worker_factory(worker, self.policy, self.env)
        ...
        for v1, v2 in zip(adapt_policy.parameters(), self.policy.parameters()):
            ...
            pytest.fail(
                "Parameters of adapted policy should not be "
