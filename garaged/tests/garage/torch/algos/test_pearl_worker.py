...
def test_methods():
    ...
    latent_dim = 5
    latent_space = akro.Box(low=-1, high=1, shape=(latent_dim,), dtype=np.float32)

    # add latent space to observation space to create a new space
    augmented_obs_space = akro.Tuple((env_spec.observation_space, latent_space))
    ...
    context_policy = ContextConditionedPolicy(
    ...
    max_path_length = 20
    worker1 = PEARLWorker(seed=1, max_path_length=max_path_length, worker_number=1)
    ...
    assert context_policy.context.shape == (1, max_path_length, encoder_input_dim)
    assert rollouts.observations.shape == (max_path_length, obs_dim)
    assert rollouts.actions.shape == (max_path_length, action_dim)
    assert rollouts.rewards.shape == (max_path_length,)
