...
@pytest.mark.parametrize("input_dim, output_dim, hidden_sizes", plain_settings)
def test_std_share_network_output_values_with_batch(
...
@pytest.mark.parametrize("input_dim, output_dim, hidden_sizes", plain_settings)
def test_std_network_output_values_with_batch(input_dim, output_dim, hidden_sizes):
    init_std = 2.0
...
def test_std_adaptive_network_output_values(
...
def test_softplus_min_std(input_dim, output_dim, hidden_sizes):
    ...
    exp_variance = torch.Tensor([min_value]).exp().add(1.0).log() ** 2
...
def test_softplus_max_std(input_dim, output_dim, hidden_sizes):
    ...
    exp_variance = torch.Tensor([max_value]).exp().add(1.0).log() ** 2

    assert torch.equal(
        dist.variance, torch.full((output_dim,), exp_variance[0], dtype=torch.float)
    )
