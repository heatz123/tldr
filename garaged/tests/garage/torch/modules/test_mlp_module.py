...
class TestMLPModel:
    ...
    def test_output_values(self, input_dim, output_dim, hidden_sizes):
        ...
        expected_output = torch.full(
            [1, output_dim], fill_value=5 * np.prod(hidden_sizes), dtype=torch.float32
        )
    ...
    def test_mlp_with_learnable_non_linear_function(self):
        ...
        module = MLPModule(
            input_dim=input_dim,
            output_dim=output_dim,
            hidden_nonlinearity=torch.nn.PReLU(init=10.0),
            hidden_sizes=hidden_sizes,
            hidden_w_init=nn.init.ones_,
            output_w_init=nn.init.ones_,
            output_nonlinearity=torch.nn.PReLU(init=1.0),
        )
