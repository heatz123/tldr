...
class TestContextConditionedPolicy:
    ...
    def setup_method(self):
        ...
        augmented_env_spec = EnvSpec(augmented_obs_space, self.env_spec.action_space)
        ...
        context_policy = TanhGaussianMLPPolicy(
            env_spec=augmented_env_spec,
            hidden_sizes=(3, 5, 7),
            hidden_nonlinearity=F.relu,
            output_nonlinearity=None,
        )

        self.module = ContextConditionedPolicy(
    ...
    def test_reset_belief(self):
        ...
        assert torch.all(torch.eq(self.module.z_means, torch.zeros(expected_shape)))
        assert torch.all(torch.eq(self.module.z_vars, torch.ones(expected_shape)))

    def test_sample_from_belief(self):
        """Test sample_from_belief."""
        self.module.sample_from_belief()
        expected_shape = [1, self.latent_dim]
        assert all([a == b for a, b in zip(self.module.z.shape, expected_shape)])
    ...
    def test_update_context(self):
        ...
        assert torch.all(
            torch.eq(self.module.context, torch.ones(updates, self.encoder_input_dim))
        )
    ...
    def test_forward(self):
        ...
        assert all([a == b for a, b in zip(policy_output[0].shape, expected_shape)])
