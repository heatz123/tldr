...
def step_env_with_gym_quirks(env, spec, n=10, render=True, serialize_env=False):
    """Step env gym helper.
    ...
    if serialize_env:
        ...
        assert round_trip.env.spec.max_episode_steps == env.env.spec.max_episode_steps
        env = round_trip
...
def choices(population, weights=None, *, cum_weights=None, k=1):
    ...
    return [
        population[bisect.bisect(cum_weights, random.random() * total, 0, hi)]
        for i in range(k)
    ]
