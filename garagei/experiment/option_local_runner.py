...
class OptionLocalRunner(LocalRunner):
    def setup(
        ...
        if sampler_cls is None:
            ...
            assert self.sampler_keys == ["option_policy", "local_option_policy"]
    ...
    def save(self, epoch, new_save=False, pt_save=False, pkl_update=False):
        ...
        if self.save_replay_buffer and epoch % 1000 == 0:
            file_name = os.path.join(
                self._snapshotter._snapshot_dir, f"replay_buffer{epoch}.pkl"
            )
            with open(file_name, "wb") as f:
                pickle.dump(self._algo.replay_buffer, f)
    ...
    def make_local_sampler(self, policy, worker_args, worker_class):
        max_path_length = self._algo.max_path_length
        seed = get_seed()
