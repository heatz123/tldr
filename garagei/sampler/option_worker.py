    ...
    def start_rollout(self):
        ...
        if "exploration_type" in self._cur_extras[self._cur_extra_idx]:
            ...
            if exploration_type == 1:
                # execute exploration policy from _starting_time
                self._starting_time = np.random.randint(0, self._max_path_length)
            elif exploration_type == 2:
                self._starting_time = 0  # execute exploration policy from the beginning
            else:
    ...
    def step_rollout(self):
        ...
        if self._path_length < cur_max_path_length:  # this is not while loop
            ...
            if exploration_type is not None:
                # exploration type
                # type 0: no exploration
                # type 1: execute exploration policy from _starting_time (tldr)
                # type 2: execute exploration policy from the beginning
