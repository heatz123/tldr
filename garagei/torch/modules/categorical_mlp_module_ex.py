...
from garagei.torch.distributions.transformed_distribution_ex import (
    TransformedDistributionEx,
)
...
class CategoricalMLPModuleEx(nn.Module):
    ...
    def forward(self, *inputs):
        ...
        if self._distribution_transformations is not None:
            dist = TransformedDistributionEx(dist, self._distribution_transformations)
    ...
    def forward_with_transform(self, *inputs, transform):
        ...
        if self._distribution_transformations is not None:
            dist_transformed = TransformedDistributionEx(
                dist_transformed, self._distribution_transformations
            )
