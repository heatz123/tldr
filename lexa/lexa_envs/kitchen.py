...
class KitchenEnv(BenchEnv):
    def __init__(
        ...
        with self.LOCK:
            self._env = KitchenMicrowaveKettleLightTopLeftBurnerV0(
                frame_skip=40, control_mode=control_mode, imwidth=width, imheight=width
            )
        ...
        self.goal_idx = 0
        self.obs_element_goals, self.obs_element_indices, self.goal_configs = (
            get_kitchen_benchmark_goals()
        )
    ...
    def _get_obs(self, state):
        image = self._env.render(
            "rgb_array", width=self._env.imwidth, height=self._env.imheight
        )
        obs = {
            "image": image,
            "state": state,
        }  # , 'image_goal': self.render_goal(), 'goal': self.goal}
        if self.log_per_goal:
            for i, goal_idx in enumerate(self.goals):
                ...
                obs["metric_success_task_relevant/goal_" + str(goal_idx)] = (
                    task_rel_success
                )
                obs["metric_success_all_objects/goal_" + str(goal_idx)] = (
                    all_obj_success
                )
        if self.use_goal_idx:
            task_rel_success, all_obj_success = self.compute_success(self.goal_idx)
            obs["metric_success_task_relevant/goal_" + str(self.goal_idx)] = (
                task_rel_success
            )
            obs["metric_success_all_objects/goal_" + str(self.goal_idx)] = (
                all_obj_success
            )
    ...
    def compute_reward(self, goal=None):
        ...
        if len(self.obs_element_indices[goal]) > 9:
            return -np.linalg.norm(
                qpos[self.obs_element_indices[goal]][9:]
                - self.obs_element_goals[goal][9:]
            )
        else:
            return -np.linalg.norm(
    ...
    def compute_success(self, goal=None):
        ...
        per_obj_success = {
            "bottom_burner": ((qpos[9] < -0.38) and (goal_qpos[9] < -0.38))
            or ((qpos[9] > -0.38) and (goal_qpos[9] > -0.38)),
            "top_burner": ((qpos[13] < -0.38) and (goal_qpos[13] < -0.38))
            or ((qpos[13] > -0.38) and (goal_qpos[13] > -0.38)),
            "light_switch": ((qpos[17] < -0.25) and (goal_qpos[17] < -0.25))
            or ((qpos[17] > -0.25) and (goal_qpos[17] > -0.25)),
